...
public class VpnService : IDisposable
{
    ...
    public Cookie TWFID => _cookieContainer.GetCookies(new Uri("https://webvpn.zju.edu.cn"))["TWFID"];
    ...
    public VpnService()
    {
        ...
        _httpClient.DefaultRequestHeaders.Add("Referer", "https://webvpn.zju.edu.cn/portal/");
    ...
    {
        var authResponse = await _httpClient.GetAsync(LoginAuthUrl);
        ...
        var loginResponse = await _httpClient.PostAsync(LoginPswUrl, content);
    ...
    public async Task<string> CheckNetwork(bool UseVpn)
    {
        ...
        try
        {
            var response = await _httpClient.GetAsync(Mirror_Url);
